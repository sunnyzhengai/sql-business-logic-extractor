...
_BRACKET_IDENT_RE = re.compile(r"\[([^\[\]]*)\]", re.DOTALL)


def _collapse_bracket_identifiers(sql: str) -> str:
    """Collapse internal whitespace inside `[bracket-quoted]` identifiers.

    T-SQL views generated by certain BI tools embed newlines inside bracket
    identifiers (e.g. `[Claim_ Month\\n_Reported]`). SQL Server treats those
    as the same identifier as `[Claim_ Month _Reported]`, but sqlglot bails
    on the embedded newline. Collapsing whitespace to a single space lets
    sqlglot parse the file -- the column names stay equivalent.
    """
    def repl(m: re.Match) -> str:
        inner = re.sub(r"\s+", " ", m.group(1)).strip()
        return f"[{inner}]"
    return _BRACKET_IDENT_RE.sub(repl, sql)
...
def collect_pairs(views_dir: str, dialect: str = "tsql") -> tuple[list[str], list[tuple[str, str]]]:
    ...
    for path in sql_files:
        ...
        try:
            ...
            # First retry: collapse whitespace inside bracket-quoted identifiers.
            # T-SQL allows multi-line `[Claim_ Month\n_Reported]` but sqlglot
            # rejects it; SQL Server treats internal whitespace variants as
            # the same name, so collapsing is safe.
            sql_normalized = _collapse_bracket_identifiers(sql)
            try:
                inv = extract_columns(sql_normalized, dialect=dialect)
                for c in inv.columns:
                    if not c.table or not c.column:
                        continue
                    tables.add(c.table)
