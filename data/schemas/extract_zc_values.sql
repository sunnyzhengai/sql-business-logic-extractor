...
-- ============================================================
-- 0. (recommended) Generate the ZC-table filter list from your
--    corpus.jsonl so the query only pulls ZC tables your views
--    actually reference (10-100x faster on large Clarity installs):
--       python -m tools.inventory_manifest.batch corpus.jsonl
--    Output:
--       inventory/zc_tables_values_clause.sql
--    Paste the contents of that file into the @TableFilter VALUES
--    block in STEP 1 below. Without a filter, the query scans every
--    ZC_* table in CLARITY_TBL (slow).
...
-- 2. Run (F5). The Messages tab will print whether the filter is
--    applied, the length of the generated dynamic SQL, and the
--    first chunk for debug. The result grid will show one row per
--    (ZC_table, code, name).
...
-- ============================================================
-- STEP 1: Paste your filter list of ZC tables here (recommended)
-- ============================================================
-- Generated by: python -m tools.inventory_manifest.batch <corpus.jsonl>
-- Output file:  inventory/zc_tables_values_clause.sql
-- Paste the rows from that file BETWEEN the VALUES and the closing ;
-- below. If you leave the list empty, the query falls back to ALL
-- ZC_* tables in CLARITY_TBL (slow on large installs).
DECLARE @TableFilter TABLE (TABLE_NAME SYSNAME);
INSERT INTO @TableFilter (TABLE_NAME) VALUES
    -- ('ZC_COVERAGE_TYPE'),
    -- ('ZC_PAT_STATUS'),
    -- ...
    ('__placeholder_so_the_INSERT_is_syntactically_valid__')
;
-- Drop the placeholder so it doesn't pollute the filter set.
DELETE FROM @TableFilter WHERE TABLE_NAME = '__placeholder_so_the_INSERT_is_syntactically_valid__';

-- ============================================================
-- STEP 2: run the rest of the query as-is
-- ============================================================
...
PRINT 'Filter applied: ' + CASE WHEN @use_filter = 1 THEN 'yes (only listed ZC tables)' ELSE 'no (all ZC_* tables)' END;
