...
if not any(p.is_dir() for p in resolve_view_source_dirs()):
    print(
        ...
        "  paths. Either move your SQL files into\n"
        "    data/views_reporting/ and data/views_cookrpt/\n"
        "  relative to your current working directory, OR pass an\n"
        "  explicit project_root to load_external_views(\n"
        "    project_root='/your/repo/path').\n"
        "  See the resolved paths above to figure out where you ARE\n"
        "  vs where the resolver is LOOKING."
    )
elif not sql_files:
    print(
