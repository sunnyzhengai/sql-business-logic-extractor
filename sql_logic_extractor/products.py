...
class ReportDescription:
    ...
    ``detailed_description`` is a parallel, richer version of the
    business_description.  It is generated by feeding the engineered
    technical scaffold (tables, columns, filters, grain) to an LLM
    and asking it to verbalize the structure into readable prose --
    grounded so it can't hallucinate beyond what the SQL actually does.
    Empty when LLM is not available or not requested.
    """
...
def _generate_report_description_core(sql: str, schema: dict, *,
                                       ...
                                       detailed: bool = False) -> ReportDescription:
    ...
    # Always compute the engineered summary -- it's the grounding scaffold
    # for both the LLM high-level summary and the detailed description.
    from .business_logic import summarize_engineered
    engineered = summarize_engineered(bl, schema or {},
                                         view_level_notes=view_notes,
                                         table_scores=table_scores)
    ...
    if use_llm:
        ...
        technical = engineered["technical_description"]
        business = engineered["business_description"]
        purpose = engineered["primary_purpose"]
        metrics = engineered["key_metrics"]

    # Grounded detailed description: LLM verbalizes the engineered scaffold.
    # Requires use_llm=True and detailed=True.
    detailed_desc = ""
    if detailed and use_llm and llm_client is not None:
        from .business_logic import summarize_detailed_llm
...
def generate_report_description(sql: str, schema: dict, *,
    ...
    `detailed=True` generates a parallel detailed_description: the LLM
    verbalizes the engineered technical scaffold into richer, multi-paragraph
    prose.  Requires `use_llm=True`; ignored otherwise."""
